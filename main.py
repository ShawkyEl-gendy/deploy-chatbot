...
def initialize_model():
    try:
        with st.spinner('Loading model...'):
            ...
            st.session_state['model'].reset_conversation()  # Clear memory on app start
            print('Model loaded successfully...')
        st.session_state['model_initialized'] = True
    except Exception as e:
        st.session_state['model_error'] = str(e)

# to make sure the model is initialized 
...
. يمكنني مساعدتك في الإجابة على استفساراتك بناءً على المعلومات المتاحة
""")
...
if st.sidebar.button("🧹 Reset Chat"):
    st.session_state['messages'] = []
    if 'model' in st.session_state and hasattr(st.session_state['model'], 'reset_conversation'):
        st.session_state['model'].reset_conversation()
    
    # Try rerun if supported
    try:
        st.rerun()
    except:
        st.success("Chat reset. Please ask a new question.")
